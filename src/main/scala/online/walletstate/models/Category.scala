...
final case class Category(id: Category.Id, namespace: Namespace.Id, name: String, createdBy: User.Id)
...
object Category {
  ...
  def make(namespace: Namespace.Id, name: String, createdBy: User.Id): UIO[Category] =
    Id.random.map(Category(_, namespace, name, createdBy))
