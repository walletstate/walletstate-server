...
import online.walletstate.common.models.{HttpError, Icon, User, Wallet}
...
trait AppError(msg: String) extends Throwable with NoStackTrace {
  ...
  def asBadRequest: HttpError.BadRequest                   = HttpError.BadRequest(errorName, message)
  def asUnauthorized: HttpError.Unauthorized               = HttpError.Unauthorized(errorName, message)
  def asForbidden: HttpError.Forbidden                     = HttpError.Forbidden(errorName, message)
  def asNotFound: HttpError.NotFound                       = HttpError.NotFound(errorName, message)
  def asConflict: HttpError.Conflict                       = HttpError.Conflict(errorName, message)
  def asInternalServerError: HttpError.InternalServerError = HttpError.InternalServerError(errorName, message)
}
