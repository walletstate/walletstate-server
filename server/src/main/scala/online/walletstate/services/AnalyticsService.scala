...
import online.walletstate.common.models.Analytics.GroupBy
import online.walletstate.common.models.{Account, Analytics, AssetAmount, Category, Group, Page, Record, Transaction}
...
import online.walletstate.{WalletUIO, common, models}
  ...
  override def records(filter: Analytics.Filter, page: Option[Page.Token]): WalletUIO[Page[Record.SingleTransaction]] =
    ZIO.serviceWithZIO[WalletContext] { ctx =>
      ...
      translate(query).orDie.debug("[Analytics] Records query") *> // Remove
        run(query).orDie.map { (rows: List[(common.models.Record, Transaction)]) =>
