...
import online.walletstate.common.models.{HttpError, User, Wallet}
...
final case class AuthRoutes(
  ...
  val loginHandler = Handler.fromFunctionZIO[Request] { req =>
    ...
    res.catchAll {
      case e: AppError.ParseRequestError  => e.asBadRequest.encodeZIO(req.outputMediaType)
      case e: AppError.InvalidCredentials => e.asForbidden.encodeZIO(req.outputMediaType)
  ...
  val changeCurrenWalletHandler = Handler.fromFunctionZIO[(Wallet.Id, Request)] { (walletId, req) =>
    ...
    rs.catchAll {
      case e: AppError.UserIsNotInWallet => e.asForbidden.encodeZIO(req.outputMediaType)
      case _                             => HttpError.InternalServerError.default.encodeZIO(req.outputMediaType)
    }
